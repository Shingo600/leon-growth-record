...
- 将来の家族共有の土台として利用可能
## Security Notes
- Cloud sync now goes through Next.js API routes instead of connecting the public browser directly to Supabase.
- Add `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `LEON_WORKSPACE_ID`, and `LEON_SYNC_PASSCODE` to Vercel before using shared sync.
- Enter the family sync code from the profile screen's `保存と共有` card to enable cloud sharing on each device.
- Re-run [supabase/schema.sql](C:\Users\desig\OneDrive\デスクトップ\codex\leon-growth-record\supabase\schema.sql) so the old anonymous read/write policies are removed.
- The old public vars `NEXT_PUBLIC_SUPABASE_ANON_KEY` and `NEXT_PUBLIC_LEON_WORKSPACE_ID` are no longer required.
